...
async def test_new_subscriber():
    ...
    assert await db.get_subscriber_mailing_time(subscriber_id=0) == mailing_time
...
async def test_change_subscriber_time():
    ...
    assert await db.get_subscriber_mailing_time(subscriber_id=0) == new_mailing_time
...
async def test_delete_subscriber():
    subscribers_before_new = await db.get_subscribers_by_mailing_time(mailing_time)
    await db.new_subscriber(subscriber_id=0, mailing_time=mailing_time)

    await db.delete_subscriber(subscriber_id=0)
    subscribers_after_delete = await db.get_subscribers_by_mailing_time(mailing_time)
